...
XCode is an Android C/C++ IDE focused on local development on mobile devices. It combines a code editor, project file tree, CMake/Ninja build workflow, clangd language features, a terminal, an Agent module, and a proot-based toolchain runtime so C/C++ projects can be edited, configured, and built directly on Android.
...
- Integrated terminal
- Agent module for future workspace-aware coding assistance
...
├── modules/agent/               Agent UI and future agent features
...
The Agent module lives in `modules/agent`. It is being built as a separate Gradle module so future provider, tool, session, and workspace-aware assistant features can evolve without coupling directly to the app shell.
...
Chinese documentation is available in [README_ZH_CN.md](README_ZH_CN.md).
