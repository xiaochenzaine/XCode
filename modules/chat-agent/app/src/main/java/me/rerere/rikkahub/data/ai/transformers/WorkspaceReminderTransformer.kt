...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("  - `apply_workspace_edits`: preferred tool for creating files and modifying existing code files; edits are tracked by the IDE transaction layer, with dirty-buffer conflict checks and diff metadata.")
    appendLine("  - `workspace_write_file`: legacy fallback for full-file writes when `apply_workspace_edits` is unavailable or unsuitable.")
    appendLine("  - `workspace_edit_file`: legacy fallback for precise text replacement when `apply_workspace_edits` is unavailable or unsuitable.")
    appendLine("  - `workspace_shell`: run shell commands (the files area is mounted at /workspace).")
    appendLine("- Prefer `apply_workspace_edits` for creating or modifying code files. Use edit type `create` for new files and `replace` for existing files. Prefer `workspace_shell` only for tasks that standard Unix tools handle well.")
    ...
    appendLine("- XCode projects are mounted at `/workspace/XCodeProjects` when external storage access is available.")
