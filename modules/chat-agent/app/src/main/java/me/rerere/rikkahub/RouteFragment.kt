    ...
    private fun ShareHandler(backStack: MutableList<NavKey>) {
        val shareIntent = remember {
            Intent().apply {
                ...
                putExtra(Intent.EXTRA_TEXT, activity?.intent?.getStringExtra(Intent.EXTRA_TEXT))
                putExtra(Intent.EXTRA_STREAM, activity?.intent?.getStringExtra(Intent.EXTRA_STREAM))
                putExtra(
                    Intent.EXTRA_PROCESS_TEXT,
                    activity?.intent?.getCharSequenceExtra(Intent.EXTRA_PROCESS_TEXT)
                )
            }
        }
