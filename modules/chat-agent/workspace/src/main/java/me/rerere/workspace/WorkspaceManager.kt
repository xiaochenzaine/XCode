...
class WorkspaceManager(
    ...
    private val sharedRootfsDir: File? = null,
) {
    ...
    fun linuxDir(root: String): File {
        requireValidRoot(root)
        return sharedRootfsDir ?: File(workspaceDir(root), LINUX_DIR)
    }

    fun tempDir(root: String): File = File(workspaceDir(root), TEMP_DIR)

    fun usesSharedRootfs(): Boolean = sharedRootfsDir != null
    ...
    fun executeCommand(
        ...
        val cwdInXcodeProjectsMount = normalizedCwd == XCODE_PROJECTS_MOUNT_RELATIVE_PATH ||
            normalizedCwd.startsWith("$XCODE_PROJECTS_MOUNT_RELATIVE_PATH/")
        if (!cwdInXcodeProjectsMount) {
    ...
    fun cleanupAllTempDirs() {
        ...
        for (dir in roots) {
            ...
            // 每个 workspace 独立 rootfs 才清理 rootfs 临时目录；共享 XCode rootfs 不在这里清理。
            if (!usesSharedRootfs()) {
    ...
    companion object {
        ...
        private const val XCODE_PROJECTS_MOUNT_RELATIVE_PATH = "XCodeProjects"
