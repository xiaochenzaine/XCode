...
fun create_line_comment_state(
    ...
    val has_actionable_line = non_blank_lines.any { (_, line_text) ->
        !is_line_commented(line_text, comment_marker) || is_toggleable_line_comment(line_text, comment_marker)
    }
    if (!has_actionable_line) return null

    val should_uncomment = non_blank_lines.all { (_, line_text) -> is_toggleable_line_comment(line_text, comment_marker) }
    return editor_line_comment_state(lines = lines, should_uncomment = should_uncomment)
}

fun is_line_commented(line_text: String, comment_marker: String = "//"): Boolean {
    val indent_length = line_comment_indent_length(line_text)
    return indent_length < line_text.length && line_text.startsWith(comment_marker, indent_length)
}

fun is_toggleable_line_comment(line_text: String, comment_marker: String = "//"): Boolean {
    val indent_length = line_comment_indent_length(line_text)
    if (indent_length >= line_text.length || !line_text.startsWith(comment_marker, indent_length)) return false

    val after_marker_index = indent_length + comment_marker.length
    val marker_next = line_text.getOrNull(after_marker_index)
    return when (comment_marker) {
