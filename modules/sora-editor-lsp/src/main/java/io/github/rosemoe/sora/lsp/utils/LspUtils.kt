...
internal fun List<Diagnostic>.transformToEditorDiagnostics(editor: CodeEditor): List<DiagnosticRegion> {
    ...
    for (diagnosticSource in this) {
        ...
        val diagnostic = DiagnosticRegion(
            ...
            id++,
            DiagnosticDetail(
                diagnosticSource.formatBriefMessage(),
                diagnosticSource.formatDetailedMessage(message),
                null,
                diagnosticSource
            )
        )
...
private fun Diagnostic.formatBriefMessage(): String {
    val severityText = severity?.name ?: "Diagnostic"
    val sourceText = source?.takeIf { it.isNotBlank() } ?: "clangd"
    val codeText = code?.let { value ->
        when {
            value.isLeft -> value.left
            value.isRight -> value.right.toString()
            else -> null
        }
    }?.takeIf { it.isNotBlank() }
    return if (codeText == null) {
        "$sourceText · $severityText"
    } else {
        "$sourceText · $severityText · $codeText"
    }
}

private fun Diagnostic.formatDetailedMessage(message: String): String {
    val location = "${range.start.line + 1}:${range.start.character + 1}"
    val details = mutableListOf("位置: $location")
    code?.let { value ->
        val codeText = when {
            value.isLeft -> value.left
            value.isRight -> value.right.toString()
    ...
    source?.takeIf { it.isNotBlank() }?.let { sourceText ->
        details += "来源: $sourceText"
    }
