...
open class TsAnalyzeManager(val languageSpec: TsLanguageSpec, var theme: TsTheme) :
    ...
    companion object {
        ...
        @Volatile
        var error_reporter: ((Throwable) -> Unit)? = null
    ...
    inner class TsLooperThread : Thread() {
        ...
        fun updateStyles() {
            runCatching {
                ...
                if (thread == this && messageQueue.isEmpty()) {
                    val contentReference = reference ?: return@onSuccess
                    val styleReceiver = currentReceiver ?: return@onSuccess
                    val parsedTree = tree ?: return@onSuccess
                    val oldTree = (styles.spans as LineSpansGenerator?)?.safeTree
                    val newTree = SafeTsTree(parsedTree.copy())
                    val newSpans = LineSpansGenerator(
                        newTree,
                        contentReference.lineCount,
                    ...
                    updateCodeBlocks()
                    styleReceiver.setStyles(this@TsAnalyzeManager, styles) {
                        styles.spans = newSpans
                        oldTree?.close()
                    }
                    styleReceiver.updateBracketProvider(
        ...
        fun updateCodeBlocks() {
            if (!languageSpec.blocksQuery.canAccess() || languageSpec.blocksQuery.patternCount == 0) {
                return
            }
