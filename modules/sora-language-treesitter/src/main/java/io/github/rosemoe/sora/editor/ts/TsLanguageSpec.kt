...
open class TsLanguageSpec(
    ...
    val blocksQuery = create_query_or_empty(language, codeBlocksScmSource)

    val bracketsQuery = create_query_or_empty(language, bracketsScmSource)

    init {
        // Check the queries before access
        try {
            if (blocksQuery !== TSQuery.EMPTY) {
                blocksQuery.validateOrThrow("code-blocks")
            }
            if (bracketsQuery !== TSQuery.EMPTY) {
            ...
            if (!tsQuery.canAccess()) {
                ...
                throw IllegalArgumentException("Syntax highlights query is invalid: ${tsQuery.errorType.name} at offset $offset")
            }
    ...
    private fun create_query_or_empty(language: TSLanguage, source: String): TSQuery {
        return if (!has_query_pattern(source)) {
            TSQuery.EMPTY
        } else {
            TSQuery.create(language, source)
        }
    }

    private fun has_query_pattern(source: String): Boolean {
