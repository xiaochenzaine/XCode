...
class TSParserInternal {
 public:
  ...
  bool begin_round(JNIEnv *env) {
    ...
    *flag = 0;
    // Tree-sitter ABI 15 removed the public ts_parser_set_cancellation_flag API.
    // Keep the Java cancellation flag state, but do not bind it to the runtime parser.
...
TSParser_setTimeout(JNIEnv *env,
                    ...
                    jlong macros) {
  req_nnp(env, parser);
  // Tree-sitter ABI 15 removed the timeout_micros API.
  // Keep Java API compatibility by ignoring timeout settings for now.
  (void) ((TSParserInternal *) parser)->getParser(env);
  (void) macros;
}
